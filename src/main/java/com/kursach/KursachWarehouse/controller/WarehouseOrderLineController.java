...
//ошибка с доступок к QR у польщователя +
//ошибка при мувинг.освободить ячейку и удалить из invent sum + после доставки занять ячейку и добавить в inventSum +
//названия вместо id
//Дизейблить стартовую или финишную ячейку и назначать по дефолту 1 + можно вывести в нее тип заказа
//изменение статуса складского заказа для пользователя +
//при отсутствии заданий , исправить разметку у пользователя - поставить 3 клетки на действия по дефолту+
//
...
public class WarehouseOrderLineController {
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    @GetMapping("/chooseWarehouseOrderType")
    public String chooseWarehouseOrderType() {

        return "chooseWarehouseOrderType";
    }


    @GetMapping("/addWarehouseOrderLine")
    @PreAuthorize("hasAuthority('ADMIN')")
    public String addWarehouseOrderLine(@RequestParam(name = "type", required = false, defaultValue = "MOVING") int type,
        ...
        {
            model.put("ROnly1","");
            model.put("ROnly2","readonly");
            model.put("DefValue1","");//стартовая
            model.put("DefValue2","1");//финишная
        }
        if (type == 3)//moving
        {
            model.put("ROnly1","readonly");
            model.put("ROnly2","");
            model.put("DefValue1","1");//стартовая
            model.put("DefValue2","");//финишная
        }//то блокируются поля
    ...
    public String changeWarehouseOrderLine(WarehouseOrderLine orderLine, Map<String, Object> model, Model model2) {
        ...
        if (authentication.getAuthorities().stream()
                .anyMatch(r -> r.getAuthority().equals("ADMIN")) == true)
            return "redirect:/warehouseOrderLine";
        else
            return "redirect:/currentUserWarehouseOrderLine";
    }
